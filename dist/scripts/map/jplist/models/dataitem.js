 ...
 */
	...
	var findPathitem = function findPathitem(self, path) {
		...
		for (var i = 0; i < self.pathitems.length; i++) {
			...
			if (jQuery.fn.jplist.services.Path.isPathsEqual(pathitem.path, path, PATH_ONLY)) {
				//path1, path2, pathOnly
 ...
 * @private
 */
	var getPathitems = function getPathitems(self, paths) {
 ...
 * @constructor 
 */
	var Init = function Init(jqElement, paths, index) {
