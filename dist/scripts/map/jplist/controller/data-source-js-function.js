...
if(self.options.cookies){jQuery.fn.jplist.services.Cookies.saveCookies(statuses,self.options.cookie_name,self.options.expiration);}if($.isFunction(self.options.dataSource.jsFunction.jsFunctionName)){ //empty html
self.$itemsBox.html(''); //show preloader
if(self.$preloader){self.$preloader.show(0);} //call to js function
self.options.dataSource.jsFunction.jsFunctionName(statuses,self.options,function(html){ //hide preloader
if(self.$preloader){self.$preloader.hide(0);} //build result html
buildHtml(self,html,statuses);if(self.options.deep_linking){ //if deep linking is enabled -> change url by statuses
self.$jplistBox.trigger(self.options.change_url_deep_links_event,[]);} //trigger collection ready event
...
if(self.options.dataSource.jsFunction.preloaderPath){self.$preloader=self.$jplistBox.find(self.options.dataSource.jsFunction.preloaderPath);} //init events
