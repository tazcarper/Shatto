 ...
 */
	...
	var buildCollectionHtml = function buildCollectionHtml(self, collection, statuses) {
 ...
 * @param {Object} self - jplist panel 'this' object
 */
	var animateToTop = function animateToTop(self) {
 ...
 * @param {Array.<jQuery.fn.jplist.models.Status>} statuses
 */
	var updateDataviewSort = function updateDataviewSort(self, collection, statuses) {
 ...
 */
	var modifyDataviewCallback = function modifyDataviewCallback(collection, status) {
		...
		collection.modifyDataview(function (dataview) {
			...
			switch (status.data.filterType) {
				...
				case 'text':
					{
						collection.dataview = jQuery.fn.jplist.actions.Filters.textFilter(status.data.value, path, dataview, status.data.ignore);
					}
					break;

				case 'path':
					{
						collection.dataview = jQuery.fn.jplist.actions.Filters.pathFilter(path, dataview);
					}
					break;

				case 'inverted_path':
					{
						collection.dataview = jQuery.fn.jplist.actions.Filters.invertedPathFilter(status.data['checked_checkboxes'], dataview);
					}
					break;

				case 'range':
					{
						collection.dataview = jQuery.fn.jplist.actions.Filters.rangeFilter(path, dataview, status.data.min, status.data.max, status.data.prev, status.data.next);
					}
					break;

				case 'date':
					{
						collection.dataview = jQuery.fn.jplist.actions.Filters.dateFilter(status.data['year'], status.data['month'], status.data['day'], path, dataview, status.data['format']);
					}
					break;

				case 'date_range':
					{
						collection.dataview = jQuery.fn.jplist.actions.Filters.dateRangeFilter(path, dataview, status.data['format'], status.data['prev_year'], status.data['prev_month'], status.data['prev_day'], status.data['next_year'], status.data['next_month'], status.data['next_day']);
					}
					break;
				...
				case 'pathGroup':
					{
						collection.dataview = jQuery.fn.jplist.actions.Filters.pathGroupFilter(status.data['pathGroup'], dataview);
					}
					break;
				...
				case 'textGroup':
					{
						collection.dataview = jQuery.fn.jplist.actions.Filters.textGroupFilter(status.data['textGroup'], status.data['logic'], status.data['path'], status.data['ignoreRegex'], dataview);
					}
					break;
				...
				case 'textFilterPathGroup':
					{
						collection.dataview = jQuery.fn.jplist.actions.Filters.textFilterPathGroup(status.data['textAndPathsGroup'], status.data['ignoreRegex'], dataview);
					}
					break;
				...
				case 'autocomplete':
					{
						collection.dataview = jQuery.fn.jplist.actions.Filters.autocompleteFilter(status.data['latitude'], status.data['longitude'], status.data['name'], dataview, status.data['radius']);
					}
					break;
			}
		});
 ...
 * @param {Array.<jQuery.fn.jplist.models.Status>} statuses
 */
	var updateDataviewFilter = function updateDataviewFilter(self, collection, statuses) {

		var filterStatuses, status;
 ...
 * @param {Array.<jQuery.fn.jplist.models.Status>} statuses
 */
	var updateDataviewPaging = function updateDataviewPaging(self, collection, statuses) {
 ...
 * @param {Array.<jQuery.fn.jplist.models.Status>} statuses
 */
	var updateDataview = function updateDataview(self, collection, statuses) {
 ...
 * @param {Object} self - jplist controller 'this' object
 */
	var initHtmlEvents = function initHtmlEvents(self) {
 ...
 */
	var addItemsToCollection = function addItemsToCollection(self, collection, $items, counter) {
 ...
 */
	var getCollection = function getCollection(self) {
 ...
 * @param {Object} self - jplist controller 'this' object
 */
	var initDataSource = function initDataSource(self) {
 ...
 * @constructor 
 */
	var Init = function Init($jplistBox, options) {
